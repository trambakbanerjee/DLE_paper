...
h.ksd.grid<- exp(seq(log(1*10^(-5)),log(0.0001),length.out=10))
...
for (N in 1:length(n.grid)){
  ...
  result<-foreach(r = 1:reps,.packages=c("CVXR","Rfast","Rlinsolve","MASS","REBayes","compoisson"))%dopar%{
    
    set.seed(10*r)
    q<-runif(n,0,1)
    set.seed(10*r)
    lam<-(q<=0.5)*rep(10,n)+(q>0.5)*rgamma(n,shape=5,scale=2)
    set.seed(100*r)
    qq<-runif(n,0,1)
    set.seed(r)
    X<- (qq<=0.8)*rpois(n,lam)+(qq>0.8)*sapply(1:n,function(i) rcom(1,lam[i],0.8))
    ...
    risk.gauss.km<-sum((delta.gauss.km-lam)^2)
    ...
    risk.tf<-sum((delta.tf-lam)^2)
    
    #--------------- Naive Estimator -----------------------
    risk.naive<-sum((X-lam)^2)
