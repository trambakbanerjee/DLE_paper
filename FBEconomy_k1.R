...
FB_Economy <- read_csv('Facebook_Economy.csv')

dat<-setNames(data.frame(matrix(ncol = 17, nrow = 29928)), c("id", "TT1", "TT2","TT3",
                                                             "TT4","TT5","TT6","TT7","TT8","PT1","PT2","PT3","PT4","PT5","PT6","PT7","PT8"))

dat$id<-FB_Economy$IDLink
for(i in 2:17){
  
  s<-i+8*(i-2)
  e<-i+8*(i-1)
  dat[,i]<-rowSums(FB_Economy[,s:e])
...
#------------------ Initializations --------------------------
reps=1
h.ksd.grid<- exp(seq(log(10^(-2)),log(0.1),length.out=10))
...
risk.ksdor.odd<-sum(((delta.ksdor.odd-odd)^2)/odd)/n
...
risk.ksd.odd<-sum(((delta.ksd.odd-odd)^2)/odd)/n
...
risk.km.odd<- sum(((km.odd-odd)^2)/odd)/n
...
risk.gauskm.odd<-sum(((delta.gauskm.odd-odd)^2)/odd)/n

# #------------- Naive --------------------
naive.suc<- X/m+0.5
naive.odd<- naive.suc/(1-naive.suc)
risk.naive.odd<- sum(((naive.odd-odd)^2)/odd)/n
...
risk.tfor.odd<-sum(((delta.tfor.odd-odd)^2)/odd)/n
