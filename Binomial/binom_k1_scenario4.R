...
h.ksd.grid<- exp(seq(log(10^(-4)),log(0.1),length.out=10))
...
for (N in 1:length(n.grid)){
  ...
  result<-foreach(r = 1:reps,.packages=c("CVXR","Rfast","Rlinsolve","MASS","REBayes","TruncatedDistributions"))%dopar%{
    
    set.seed(10*r)
    q<-runif(n,0,1)
    set.seed(r^2)
    sp<- (q<=0.5)*rtbeta(n,1,1,a=0.1,b=0.8)+(q>0.5)*rtbeta(n,1,3,a=0.1,b=0.8)
    set.seed(r)
    X<- rbinom(n,m,sp)
    odd<- sp/(1-sp)
    ...
    for(i in 1:length(h.ksd.grid)){
      h.ksd<-h.ksd.grid[i]
      out<-tryCatch(ksd_binom_k1_new(X,h.ksd,n,m),
                    error=function(e) NULL)
      step1.p<- out$delt.p
      Xp<-out$xp
      mp<-out$mp
      step1<- out$delt
      out.ure<-ure_binom_new(X,Xp,step1,step1.p,m,mp,h.ksd,1)
      ure.delta[i]<-out.ure$ure*(!is.null(out))+10^12*(is.null(out))
      loss.delta[i]<-sum(((step1-odd)^2)/odd)*(!is.null(step1))+10^12*(is.null(step1))
    }
    ...
    #--------------- NEB h OR -----------------------
    out.ksd<- ksd_binom_k1_new(X,h.ksd.or,n,m)
    w.hat.or<-out.ksd$w.hat
    delta.ksdor.odd<-out.ksd$delt
    delta.ksdor.suc<- delta.ksdor.odd/(1+delta.ksdor.odd)
    risk.ksdor.odd<-sum(((delta.ksdor.odd-odd)^2)/odd)
    risk.ksdor.suc<-sum(((delta.ksdor.suc-sp)^2)/sp)
    
    #--------------- NEB hest -----------------------
    out.ksd<- ksd_binom_k1_new(X,h.ksd.est,n,m)
    w.hat<-out.ksd$w.hat
    delta.ksd.odd<-out.ksd$delt
    delta.ksd.suc<-delta.ksd.odd/(1+delta.ksd.odd)
    risk.ksd.odd<-sum(((delta.ksd.odd-odd)^2)/odd)
    risk.ksd.suc<-sum(((delta.ksd.suc-sp)^2)/sp)
    
    #------------- KM ----------------
    out.km<- Bmix(X,m,v=300,collapse=FALSE)
    km.dy<- predict(out.km,X,Loss=2,m)
    km.odd<- km.dy/(1-km.dy)
    risk.km.odd<- sum(((km.odd-odd)^2)/odd)
    risk.km.suc<- sum(((km.dy-sp)^2)/sp)
    ...
    risk.gauskm.suc<-sum(((delta.gauskm.suc-sp)^2)/sp)
    risk.gauskm.odd<-sum(((delta.gauskm.odd-odd)^2)/odd)
    ...
    risk.tfor.suc<-sum(((delta.tfor.suc-sp)^2)/sp)
    risk.tfor.odd<-sum(((delta.tfor.odd-odd)^2)/odd)
    
    # #------------- Naive --------------------
    naive.suc<- X/m
    naive.odd<- naive.suc/(1.5-naive.suc)
    risk.naive.odd<- sum(((naive.odd-odd)^2)/odd)
    risk.naive.suc<- sum(((naive.suc-sp)^2)/sp)
